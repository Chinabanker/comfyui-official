...
# Pinned to v0.37.4 (latest release tag as of 2026-09-26)
WORKDIR /app
RUN git clone --depth 1 --branch v0.37.4 https://github.com/Comfy-Org/ComfyUI.git /app/ComfyUI \
    && PIP_INDEX_URL="${PIP_INDEX_URL:-}" pip install -r /app/ComfyUI/requirements.txt \
    && PIP_INDEX_URL="${PIP_INDEX_URL:-}" pip install --upgrade comfy-aimdo==0.5.5
